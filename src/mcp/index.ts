/**
 * Agent API Gateway — MCP server (stdio)
 *
 * Claude Desktop / Cursor / VS Code Copilot:
 *   AGENT_API_KEY=sk-... npx tsx src/mcp/index.ts
 * or after build:
 *   AGENT_API_KEY=sk-... node dist/mcp/index.js
 *
 * Never commit AGENT_API_KEY. Keys stay in env only.
 */
...
if (!API_KEY) {
  // stderr only — MCP uses stdout for the protocol
...
const server = new Server(
  { name: 'agent-api-gateway', version: '0.2.0' },
  { capabilities: { tools: {} } },
);
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    const message = safeErrorMessage(err, 'MCP tool failed');
    return {
      isError: true,
...
async function main() {
  ...
  console.error('[mcp] agent-api-gateway stdio server ready');
}
